...
namespace spray {
  ...
  // It creates a "special" BlockReduction that actually just keeps a pointer to the
  // original data array, and behaves like a BlockReduction with just one block
  ...
  // BlockReduction, it is called at the end of the scope where this is created by the
  // user.
      ~BlockReduction() {
  ...
  // Function to obtain the peak memory consumption of this BlockReduction. If this is
  ...
  // BlockReduction `orig` that wraps the original data. The `init` object allocates a
  // list of pointers to blocks, but leaves the individual blocks un-allocated.
      static void ompInit(BlockReduction<contentType> *init, BlockReduction<contentType> *orig) {
  ...
  //  - The output may be the "special" BlockReduction that holds the original array.
  ...
  //       block is aligned, and we make sure that the output BlockReduction will
  //       obtain the ownership of this original block, even if it belonged to the
  //       input BlockReduction (which will fall out of scope after this and will no
  //       longer need the lock to that block anyways).
      static void ompReduce(BlockReduction<contentType> *out, BlockReduction<contentType> *in) {
        ...
        if(out->nblocks == 1 && out->blocks[0] != NULL) {
          ...
          // We are merging two BlockReductions, neither of which is the "special" one
          for(i=0; i<in->nblocks; i++) {
            if(in->blocks[i] != NULL) {
  ...
  // actually just pointers to the original array, wich may be unaligned.
      static void addBlock(contentType* curOut, contentType* curIn, int block, int totalsize, bool outAligned) {
  ...
  // array held by the "special" BlockReduction. If successful, this thread will
