...
namespace spray {
  ...
  template <typename contentType> class AtomicReduction {
    public:
      ...
      static void ompInit(AtomicReduction<contentType> *init, AtomicReduction<contentType> *orig) {
        assert(orig->initialized);
        init->orig = orig->orig;
        init->initialized = true;
      }
  
      static void ompReduce(AtomicReduction<contentType> *out, AtomicReduction<contentType> *in) {
