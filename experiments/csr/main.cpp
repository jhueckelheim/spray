...
void spmvt_awlock(csr<real> csr_data, real* res, real* x) {
    spray::BlockReduction1024<real> x_p(csr_data.nc, x);
    #pragma omp parallel for reduction(+:x_p)
    for (int i = 0; i < csr_data.nr; i++) {
        for (int k = csr_data.rptr[i]; k < csr_data.rptr[i+1]; k++) {
            x_p[csr_data.cols[k]] += csr_data.vals[k]*res[i];
...
void spmvt_mkl_ie_nohint(csr<double> csr_data, double* res, double* x) {
    sparse_matrix_t A;
    matrix_descr descr;
    descr.type = SPARSE_MATRIX_TYPE_GENERAL;
    if(SPARSE_STATUS_SUCCESS != mkl_sparse_d_create_csr(&A, SPARSE_INDEX_BASE_ZERO, csr_data.nr, csr_data.nc, csr_data.rptr, csr_data.rptr+1, csr_data.cols, csr_data.vals) ||
...
void spmvt_mkl_ie_nohint(csr<float> csr_data, float* res, float* x) {
    sparse_matrix_t A;
    matrix_descr descr;
    descr.type = SPARSE_MATRIX_TYPE_GENERAL;
    if(SPARSE_STATUS_SUCCESS != mkl_sparse_s_create_csr(&A, SPARSE_INDEX_BASE_ZERO, csr_data.nr, csr_data.nc, csr_data.rptr, csr_data.rptr+1, csr_data.cols, csr_data.vals) ||
...
int main (int argc,char **argv){
    if(argc < 3) {
      printf("Usage: ./exec <matrix_file_name> <num_iter> <method>\n");
      exit(1);
    }
    ...
    switch (method)
    {
      ...
      case 11:
        ...
        break;
      case 12:
        // Sparse Transpose-Matrix Vector Product using AWBlock Reduction
        for (int c = 0; c <count; c++){
            spmvt_awlock(csr_data, res, x);
        }
        time = omp_get_wtime();
        for (int c = 0; c <count; c++){
            spmvt_awlock(csr_data, res, x);
        ...
        time = omp_get_wtime() - time;
        printf("sptmv_awlock time on %d threads: %f\n",omp_get_max_threads(),time);
        break;
#ifdef __INTEL_COMPILER
      case 13:
      ...
      case 15:
        // Sparse Transpose-Matrix Vector Product using Intel MKL
        for (int c = 0; c <count; c++){
            spmvt_mkl_ie_nohint(csr_data, res, x);
        }
        time = omp_get_wtime();
        for (int c = 0; c <count; c++){
            spmvt_mkl_ie_nohint(csr_data, res, x);
...
#endif
      default:
        std::cout<<"Unknown method, choose 1-14 (or 1-12 if not using Intel compiler)."<<std::endl;
    }
